import { useEffect, useMemo, useState } from 'react'
import PropTypes from 'prop-types'
import { Image, Select, Spin } from 'antd'
import { getEnv } from '../../../utils/env'
import tenorGif from '../../../assets/images/tenor.gif'

const STORE_MENUS_URL = 'https://gusty1.github.io/Database/order-drink/storeMenus.json'

const FoodMenu = ({ storeName }) => {
  const [storeMenu, setStoreMenu] = useState(storeName)
  const [storeNames, setStoreNames] = useState([])
  const [loading, setLoading] = useState(true)
  const env = getEnv()

  useEffect(() => {
    fetch(STORE_MENUS_URL)
      .then((res) => res.json())
      .then((data) => setStoreNames(data))
      .catch((err) => console.error('無法載入飲料店資料', err))
      .finally(() => setLoading(false))
  }, [])

  // M3: 改用 matchMedia 避免直接讀取 window.innerWidth，useMemo 確保不重複計算
  const imageHeight = useMemo(() => {
    return window.matchMedia('(max-width: 768px)').matches ? 280 : 500
  }, [])

  const changeMenu = (menu) => {
    setStoreMenu(menu)
  }

  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', padding: '40px 0' }}>
        <Spin size="large" />
      </div>
    )
  }

  if (!storeMenu) return null

  // M1: find 結果加 guard，避免 undefined.url runtime crash
  const found = storeNames.find((item) => item.value === storeMenu)
  if (!found) {
    return <div>找不到對應的菜單，請確認 REACT_APP_STORE_NAME 設定</div>
  }

  return (
    <>
      <Select
        value={storeMenu}
        style={{ marginBottom: 12, width: '100%' }}
        onChange={changeMenu}
        options={storeNames}
        disabled={env.REACT_APP_DISABLED_MENU}
        showSearch
        filterOption={(input, option) =>
          option.label.toLowerCase().includes(input.toLowerCase())
        }
      />
      <Image
        width="100%"
        height={imageHeight}
        src={found.url}
        style={{
          objectFit: 'contain'
        }}
        fallback="data:image/png;base64,"
        placeholder={
          // L9: 改用本地 GIF，避免內網環境無法存取外部 URL
          <Image
            preview={false}
            src={tenorGif}
            width="100%"
            height="100%"
            style={{
              objectFit: 'cover'
            }}
          />
        }
      />
    </>
  )
}

FoodMenu.propTypes = {
  storeName: PropTypes.string.isRequired
}

export default FoodMenu
